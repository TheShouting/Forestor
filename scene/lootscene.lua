...
function lootscene:_init(player)
   ...
   if loot.rot and  loot.rot > 0 then
      destroybutton = 
         ...
         -self.border, self.border, 128, 32,
         1, 0, 1, 0)
   end
   ...
   self.right = button(rmsg, self.takeright, 
      -10, 0, 128, 32,
      0.5, 0.5, 1, 0.5)
   self.left = button(lmsg, self.takeleft, 
      10, 0, 128, 32,
      0.5, 0.5, 0, 0.5)
   ...
   scene._init(self, 
      button("CLOSE", self.close, 
         self.border, self.border, 128, 32,
         0, 0, 0, 0),
      destroybutton, self.right, self.left)

end

function lootscene:close()
